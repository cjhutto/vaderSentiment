...
def sentiment(text):
    ...
    # get rid of empty items or single letter "words" like 'a' and 'I' from
    # wordsOnly
    for word in wordsOnly:
        if len(word) <= 1:
            wordsOnly.remove(word)
    # now remove adjacent & redundant punctuation from [wordsAndEmoticons]
    # while keeping emoticons and contractions
    puncList = [".", "!", "?", ",", ";", ":", "-", "'", "\"",
                "!!", "!!!", "??", "???", "?!?", "!?!", "?!?!", "!?!?"]
    for word in wordsOnly:
        for p in puncList:
            ...
            while x2 > 0:
                ...
                wordsAndEmoticons.insert(i, word)
                x2 = wordsAndEmoticons.count(wordp)
    # get rid of residual empty items or single letter "words" like 'a' and
    # 'I' from wordsAndEmoticons
    ...
    def wildCardMatch(patternWithWildcard, listOfStringsToMatchAgainst):
        listOfMatches = fnmatch.filter(
            listOfStringsToMatchAgainst, patternWithWildcard)
        ...
        if item_lowercase in word_valence_dict:
            ...
            def scalar_inc_dec(word, valence):
                ...
                if word_lower in booster_dict:
                    ...
                    if valence < 0:
                        scalar *= -1
                    # check if booster/dampener word is in ALLCAPS (while
                    # others aren't)
            ...
            if i > 1 and str(wordsAndEmoticons[i - 2]).lower() not in word_valence_dict:
                ...
                if s2 != 0:
                    s2 = s2 * 0.95
                v = v + s2
                # check for special use of 'never' as valence modifier instead
                # of negation
            ...
            if i > 2 and str(wordsAndEmoticons[i - 3]).lower() not in word_valence_dict:
                ...
                # check for special case idioms using a sentiment-laden keyword
                # known to SAGE
                ...
                twoonezero = "{} {} {}".format(str(wordsAndEmoticons[
                                               i - 2]), str(wordsAndEmoticons[i - 1]), str(wordsAndEmoticons[i]))
                twoone = "{} {}".format(
                    str(wordsAndEmoticons[i - 2]), str(wordsAndEmoticons[i - 1]))
                threetwoone = "{} {} {}".format(str(wordsAndEmoticons[
                                                i - 3]), str(wordsAndEmoticons[i - 2]), str(wordsAndEmoticons[i - 1]))
                threetwo = "{} {}".format(
                    str(wordsAndEmoticons[i - 3]), str(wordsAndEmoticons[i - 2]))
                ...
                if len(wordsAndEmoticons) - 1 > i:
                    zeroone = "{} {}".format(
                        str(wordsAndEmoticons[i]), str(wordsAndEmoticons[i + 1]))
                    if zeroone in special_case_idioms:
                        v = special_case_idioms[zeroone]
                if len(wordsAndEmoticons) - 1 > i + 1:
                    zeroonetwo = "{} {}".format(str(wordsAndEmoticons[i]), str(
                        wordsAndEmoticons[i + 1]), str(wordsAndEmoticons[i + 2]))
                    if zeroonetwo in special_case_idioms:
                        v = special_case_idioms[zeroonetwo]

                # check for booster/dampener bi-grams such as 'sort of' or
                # 'kind of'
